...
async function importData() {
  ...
  try {
    ...
    // 读取并执行SQL创建表
    console.log('创建数据库表...');
    const sqlFile = fs.readFileSync(path.join(__dirname, 'init-real-data.sql'), 'utf8');
    ...
    // 清空现有数据
    console.log('清空现有数据...');
    await connection.execute('DELETE FROM hr_resume_access');
    await connection.execute('DELETE FROM hr_job_match');
    await connection.execute('DELETE FROM hr_job_requirement');
    await connection.execute('DELETE FROM hr_talent');
    console.log('现有数据已清空');
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        const status = row['人员状态'] ? row['人员状态'].trim() : '在职';
